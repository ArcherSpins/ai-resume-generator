  ...
  useEffect(() => {
    ...
    try {
      ...
      renderAsync(bytes.buffer, el, undefined).catch((err) => {
        el.innerHTML = `<p class="p-4 text-danger text-sm">Preview error: ${err.message}</p>`;
      });
    } catch (err) {
      el.innerHTML = `<p class="p-4 text-danger text-sm">${err.message}</p>`;
    }
  }, [docxBase64]);

  return (
    <div
      className="w-full bg-preview rounded-lg sm:rounded-xl shadow-inner overflow-auto overscroll-contain -mx-1 sm:mx-0"
...
<body>${html}</body>
  ...
  return (
    <div
      className="w-full bg-preview rounded-lg sm:rounded-xl overflow-auto overscroll-contain -mx-1 sm:mx-0"
      style={{ maxHeight: 'min(85vh, 900px)' }}
      role="region"
...
export default function CreateResumePage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 pb-24 sm:pb-28">
      {/* Page heading */}
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-ink">{t('createResume')}</h1>
        <p className="mt-1 text-ink-muted text-sm sm:text-base">{t('createResumeSubtitle')}</p>
      </div>
      ...
      >
        ...
        <span className="mt-2 hidden sm:block text-xs font-semibold text-voice bg-surface/95 dark:bg-surface-2/95 rounded-full px-3 py-1 border border-voice-soft/50 shadow-card text-center group-hover:translate-y-[-1px] transition-transform">
          {t('generateByVoice')}
        </span>
      ...
      <section className="rounded-xl sm:rounded-2xl border border-edge bg-surface p-4 sm:p-6 shadow-card relative">
        <h2 className="text-base sm:text-lg font-semibold text-ink mb-3 sm:mb-4">
          {t('uploadExample')}
        </h2>
        {uploadLoading ? (
          <div className="rounded-xl border-2 border-dashed border-edge bg-surface-2/50 py-8 sm:py-12 flex flex-col items-center justify-center">
            <div className="animate-spin rounded-full h-10 w-10 border-2 border-primary border-t-transparent" />
            <p className="mt-3 text-ink-muted">{t('analyzingResume')}</p>
          </div>
        ) : (
          <>
            <div className="relative">
              ...
              <div
                className="absolute inset-0 rounded-xl bg-overlay/60 backdrop-blur-[2px] flex flex-col items-center justify-center text-white z-10"
                aria-hidden
              >
                ...
                <button
                  ...
                  className="mt-4 rounded-xl bg-primary text-on-primary px-5 py-2.5 text-sm font-semibold hover:opacity-90 transition shadow-lg border border-primary/20"
                >
                  {t('chooseTemplate')}
                </button>
              </div>
      ...
      {/* ── B. Template preview ────────────────────────────────────────────── */}
      {hasPreview && (
        <section className="rounded-xl sm:rounded-2xl border border-edge bg-surface shadow-card overflow-hidden">
          {/* Header bar: compact on mobile */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 px-3 sm:px-6 py-3 border-b border-edge-subtle bg-surface-2/60">
            <div className="flex items-center gap-2 min-w-0">
              <span className="inline-flex flex-shrink-0 items-center justify-center w-6 h-6 rounded-full bg-primary-soft text-primary text-xs font-bold">
                B
              </span>
              <h2 className="text-sm sm:text-lg font-semibold text-ink truncate">
                {t('templatePreview')}
              </h2>
            </div>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  p{margin-bottom:2px;line-height:1.5;}
                ...
                className="text-xs sm:text-sm text-primary hover:text-primary transition font-medium flex items-center gap-1 py-1.5 touch-manipulation"
              >
              ...
              <button
                ...
                className="text-xs sm:text-sm text-ink-muted hover:text-ink-muted transition font-medium py-1.5 touch-manipulation"
              >
      ...
      <section className="rounded-xl sm:rounded-2xl border border-edge bg-surface p-4 sm:p-6 shadow-card">
        <h2 className="text-base sm:text-lg font-semibold text-ink mb-3 sm:mb-4">
          {t('templateSelection')}
        </h2>
        <p className="text-ink-muted text-sm mb-4">{t('selectTemplateHint')}</p>
        <button
          type="button"
          onClick={() => setTemplateModalOpen(true)}
          className="w-full sm:w-auto rounded-xl bg-primary-soft text-primary px-4 py-2.5 sm:py-2 font-medium hover:bg-primary-soft transition border border-primary/35"
        >
          {t('selectTemplate')}
        </button>
        <Modal
        ...
        >
          {templateLoading && !templates.length ? (
            ...
            <>
              <div className="flex rounded-lg bg-surface-2 p-0.5 mb-3 sm:mb-4">
                <button
                  type="button"
                  onClick={() => setTemplateTab('docx')}
                  className={`flex-1 rounded-md px-2 sm:px-3 py-2 text-xs sm:text-sm font-medium transition touch-manipulation ${templateTab === 'docx' ? 'bg-surface text-ink shadow-card' : 'text-ink-muted hover:text-ink'}`}
                >
                  {t('tabWord')}
                </button>
                <button
                  ...
                  className={`flex-1 rounded-md px-2 sm:px-3 py-2 text-xs sm:text-sm font-medium transition touch-manipulation ${templateTab === 'xlsx' ? 'bg-surface text-ink shadow-card' : 'text-ink-muted hover:text-ink'}`}
                >
                  {t('tabExcel')}
                </button>
              </div>
              <div key={templateTab} className="grid grid-cols-1 gap-2 sm:gap-3 max-w-full overflow-x-hidden">
                ...
                <button
                  ...
                  className="rounded-lg sm:rounded-xl border border-edge bg-surface overflow-hidden text-left hover:border-primary hover:shadow-lg hover:shadow-card transition disabled:opacity-50 focus:ring-2 focus:ring-primary focus:ring-offset-0 focus:outline-none w-full touch-manipulation"
                >
                  <div className="flex items-center justify-between gap-2 px-3 sm:px-4 py-2.5 sm:py-3 bg-gradient-to-r from-surface-2 to-surface border-b border-edge-subtle">
                  ...
                  <div className="p-2 sm:p-4 min-h-[88px] sm:min-h-[110px] flex items-center justify-center relative">
                    {tmpl.type === 'docx' ? (
                      ...
                      tmpl.id.startsWith('rirekisho') ? (
                        <div className="w-full max-w-[200px] sm:max-w-[240px] mx-auto bg-white text-slate-700 rounded-sm overflow-hidden shadow-lg border border-slate-200" style={{ boxShadow: '0 4px 14px rgba(0,0,0,0.08), 0 1px 3px rgba(0,0,0,0.06)' }}>
                          <div className="px-2 py-1 bg-blue-50/80 border-b border-edge flex items-center gap-1" style={{ fontSize: '9px' }}>
                          ...
                          <table className="w-full border-collapse" style={{ tableLayout: 'fixed', fontSize: '10px' }}>
                            <tbody>
                              <tr><td className="border border-slate-400 bg-slate-100 font-bold py-1 px-1.5 text-center" colSpan={3}>履歴書</td></tr>
                              <tr><td className="border border-slate-400 bg-slate-100 w-14 py-0.5 px-1">日付</td><td className="border border-slate-400 py-0.5 px-1 text-slate-400">年月日</td><td className="border border-slate-400 w-12 py-0.5 text-center text-slate-500">写真</td></tr>
                              <tr><td className="border border-slate-400 bg-slate-100 py-0.5 px-1">氏名</td><td className="border border-slate-400 py-0.5 px-1"></td><td rowSpan={4} className="border border-slate-400 bg-slate-200/80"></td></tr>
                              <tr><td className="border border-slate-400 bg-slate-100 py-0.5 px-1">ふりがな</td><td className="border border-slate-400 py-0.5 px-1"></td></tr>
                              <tr><td className="border border-slate-400 bg-slate-100 py-0.5 px-1">生年月日</td><td className="border border-slate-400 py-0.5 px-1"></td></tr>
                              <tr><td className="border border-slate-400 bg-slate-100 py-0.5 px-1">現住所</td><td className="border border-slate-400 py-0.5 px-1"></td></tr>
                              <tr><td className="border border-slate-400 bg-slate-100 py-0.5 px-1" colSpan={3}>学歴・職歴</td></tr>
                              <tr><td className="border border-slate-400 w-8 text-center py-0.5">年</td><td className="border border-slate-400 w-8 text-center py-0.5">月</td><td className="border border-slate-400 py-0.5 px-1">学歴・職歴</td></tr>
                            </tbody>
                          </table>
                        </div>
                      ) : (
                        <div className="w-full max-w-[200px] sm:max-w-[240px] mx-auto bg-white text-slate-700 rounded-sm overflow-hidden shadow-lg border border-slate-200" style={{ boxShadow: '0 4px 14px rgba(0,0,0,0.08), 0 1px 3px rgba(0,0,0,0.06)' }}>
                          ...
                          <table className="w-full border-collapse" style={{ tableLayout: 'fixed', fontSize: '10px' }}>
                            <tbody>
                              <tr><td className="border border-slate-400 bg-slate-100 font-bold py-1.5 px-2 text-center" colSpan={2}>職務経歴書</td></tr>
                              <tr><td className="border border-slate-400 bg-slate-100 w-16 py-0.5 px-1">氏名</td><td className="border border-slate-400 py-0.5 px-1"></td></tr>
                              <tr><td className="border border-slate-400 bg-slate-100 py-0.5 px-1">メール</td><td className="border border-slate-400 py-0.5 px-1"></td></tr>
                              <tr><td className="border border-slate-400 bg-blue-50/80 font-semibold py-0.5 px-1">1. キャリアサマリー</td><td className="border border-slate-400 py-0.5 px-1"></td></tr>
                              <tr><td className="border border-slate-400 bg-blue-50/80 font-semibold py-0.5 px-1">2. 職務経歴</td><td className="border border-slate-400 py-0.5 px-1"></td></tr>
                              <tr><td className="border border-slate-400 bg-blue-50/80 font-semibold py-0.5 px-1">3. 学歴</td><td className="border border-slate-400 py-0.5 px-1"></td></tr>
                            </tbody>
                          </table>
                        </div>
                      )
                      ...
                      tmpl.id.startsWith('rirekisho') ? (
                        <div className="w-full max-w-[220px] sm:max-w-[260px] mx-auto overflow-hidden rounded-md border-2 border-green-200 bg-white text-slate-700" style={{ fontSize: '10px' }}>
                          ...
                          <div className="border-b border-r border-green-200/80 flex bg-slate-100">
                            <div className="w-6 flex-shrink-0 border-r border-green-200/80 bg-slate-100" />
                            <div className="flex flex-1">
                              <div className="w-12 border-r border-green-200/80 py-0.5 text-center text-slate-600 font-medium">A</div>
                              <div className="w-14 border-r border-green-200/80 py-0.5 text-center text-slate-600 font-medium">B</div>
                              <div className="flex-1 py-0.5 text-center text-slate-600 font-medium">C</div>
                            </div>
                          </div>
                          {[1, 2, 3, 4, 5, 6, 7].map((row) => (
                            <div key={row} className="flex border-b border-green-200/60">
                              <div className="w-6 flex-shrink-0 border-r border-green-200/80 bg-slate-100 py-0.5 text-center text-slate-400 text-[9px]">{row}</div>
                              <div className="flex-1 flex">
                                <div className="w-12 border-r border-slate-300 py-0.5 px-0.5">{row === 1 ? '日付' : row === 2 ? '氏名' : row === 3 ? 'ふりがな' : row === 4 ? '生年月日' : row === 5 ? '現住所' : row === 6 ? '学歴' : ''}</div>
                                <div className="w-14 border-r border-slate-300 py-0.5 px-0.5 text-slate-400"></div>
                        ...
                        <div className="w-full max-w-[220px] sm:max-w-[260px] mx-auto overflow-hidden rounded-md border-2 border-green-200 bg-white text-slate-700" style={{ fontSize: '10px' }}>
                          ...
                          <div className="border-b border-r border-green-200/80 flex bg-slate-100">
                            <div className="w-6 flex-shrink-0 border-r border-green-200/80 bg-slate-100" />
                            <div className="flex flex-1">
                              <div className="w-20 border-r border-green-200/80 py-0.5 text-center text-slate-600 font-medium">A</div>
                              <div className="flex-1 py-0.5 text-center text-slate-600 font-medium">B</div>
                            </div>
                          </div>
                          {[1, 2, 3, 4, 5, 6].map((row) => (
                            <div key={row} className="flex border-b border-green-200/60">
                              <div className="w-6 flex-shrink-0 border-r border-green-200/80 bg-slate-100 py-0.5 text-center text-slate-400 text-[9px]">{row}</div>
                              <div className="flex-1 flex">
                                <div className="w-20 border-r border-slate-300 py-0.5 px-0.5">{row === 1 ? '氏名' : row === 2 ? 'メール' : row === 3 ? '1.キャリア' : row === 4 ? '2.職務経歴' : row === 5 ? '3.学歴' : '4.資格'}</div>
      ...
      {schema && (
        <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8" noValidate>
          <section className="rounded-xl sm:rounded-2xl border border-edge bg-surface p-4 sm:p-6 shadow-card">
            <h2 className="text-base sm:text-lg font-semibold text-ink mb-4">
              {t('yourInformation')}
            </h2>
                  ...
                  {avatarPreviewUrl ? (
                    <div className="relative flex-shrink-0">
                      <img
                        ...
                        alt=""
                        className="h-24 w-20 rounded-lg border border-edge object-cover shadow-sm"
                    ...
                    <div className="flex-shrink-0 h-24 w-20 rounded-lg border-2 border-dashed border-edge bg-surface-2 flex flex-col items-center justify-center text-ink-faint text-xs">
          ...
          <section className="rounded-xl sm:rounded-2xl border border-edge bg-surface p-4 sm:p-6 shadow-card">
            ...
            <button
              ...
              className="w-full sm:w-auto rounded-xl bg-primary text-on-primary font-semibold px-8 py-3 hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition shadow-card"
            >
              {submitLoading ? (
      ...
      {!schema && !uploadLoading && (
        <div className="rounded-xl border border-dashed border-edge bg-surface-2 p-6 text-center text-ink-faint text-sm">
          {t('uploadHint')}
        </div>
      )}
    </div>
